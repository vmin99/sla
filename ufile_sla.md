...
1.1 US3对象存储数据存储的持久性为99.999999999％。
...
7.1 UCloud
US3提供给用户的服务功能为www.ucloud.cn 网站所展示的对象存储服务，完整的功能介绍和操作说明详见产品使用手册和帮助说明。
...
8.1
本公司向您承诺US3对象存储的标准存储类型服务可用性为99.95％，低频和归档存储类型服务可用性为99.9%。如果服务可用性没有达到承诺，将按照服务赔偿条款进行赔付。

8.2
失败请求定义：HTTP状态码为5XX的请求和因为US3服务故障导致用户正常请求不能到达US3服务器的请求。但不包括以下请求类型：  
8.2.1 获取Bucket列表产生的失败请求；  
8.2.2 镜像回源超时的请求；  
8.2.3 Callback超时的请求；  
8.2.4 客户应用程序收到黑客攻击而被US3限制的请求；  
8.2.5 生命周期和跨区域复制后端异步处理的失败请求；  
8.2.6 采用不适当的访问模式导致请求被US3限制的错误请求（HTTP状态码503）；  
8.2.7 因客户端网络问题或公网物理网络链路不稳定使请求不可达导致失败；  
8.2.8 由US3服务发起的合理升级、变更、停机而导致的错误请求或服务不可用情况。  

8.3
服务可用性定义：合同期内用户每月使用US3服务的时长，按5分钟的时间窗口进行划分，根据每5分钟内用户通过US3提供的API、SDK、控制台或者管理工具等方式对US3进行的操作请求次数和失败请求次数计算出“每5分钟错误率”。每5分钟错误率=5分钟内失败请求次数/5分钟内有效请求总次数*100%。

用户每月的服务可用性=（1-用户当月的每5分钟错误率之和/用户当月使用US3服务时长中包含的5分钟窗口总数）*100%。

## 9、业务资源调配能力

9.1 US3空间支持在线升级扩展，单位为GB，具体资源范围和资费详见订单页面展示；
